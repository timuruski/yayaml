...
RSpec.describe Yayaml::Command do
  describe ".build" do
    it "reads configuration from ARGV" do
      subject = described_class.build(%w[--match-path --case-sensitive hello spec/fixtures])

      expect(subject.case_sensitive).to eq(true)
      expect(subject.match_path).to eq(true)
